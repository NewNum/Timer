...
@Preview
@Composable
private fun ByKeyDraggableLazyColumnDemo() {
        ...
        draggableItems(
            ...
            ListItem(
                colors = ListItemDefaults.colors(
                    containerColor = if (isDragging) Color.LightGray else Color.White
                ),
                leadingContent = {
                    Icon(
...
@Preview
@Composable
private fun WithMovableHeaderDraggableLazyColumnDemo() {
    ...
    LazyColumn(
        ...
        draggableItems(
            ...
            items = list,
            key = { item -> item },
            contentType = { item -> item % 10 == 0 } // Make a header if it's divided by 10
        ) { item, isDragging ->
            if (item % 10 != 0) {
                ListItem(
                    colors = ListItemDefaults.colors(
...
@Preview
@Composable
private fun ByIndexDraggableLazyColumnDemo() {
